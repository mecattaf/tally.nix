...
impl SystemdNotifier {
    ...
    /// fresh `budget` measured from receipt, so the limit stops being "how long
    /// may the whole of `Daemon::open` take" and becomes "how long may any one
    /// phase of it take". A daemon whose estate has grown keeps starting; one
    /// wedged inside a phase still dies on a clock the operator already
    /// recognises. systemd ignores the field outside a start, stop or reload
    /// job, so sending it is safe wherever the notifier reaches.
