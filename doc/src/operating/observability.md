...
attempt and a node the run attached rather than created is inside the sum. The denominator comes
from each row's durable attempt counter, falling back to the canonical witness only when detail is
