...
| An armed campaign will not list, poll, or re-arm after an upgrade/GC | `campaign flow asset is missing for registration … arm …` or the corresponding `driver` error | A pre-ownership registration's unrooted asset was already removed. Tally will not substitute current machinery under the approved digest. Restore the exact asset if available; otherwise inspect, explicitly `campaign disarm`, and arm again with reviewed assets. Do not hand-edit `campaigns/armed/` or `campaigns/assets/`. |
