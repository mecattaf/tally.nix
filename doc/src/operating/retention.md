...
| Archived captures | Per-attempt `.out` and raw `.adapter.err`; failed attempts may include the bounded `.err` projection, and campaign checkpoint attempts include a `checkpoint.json` with stdout and stderr each bounded to 8 KiB | Coordinator `tally gc` prunes files older than `captureArchiveHorizon` (30 days by default). Witnesses do not pin them. Remote-worker state needs its own policy. |
