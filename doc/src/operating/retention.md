...
| `flow-membership.jsonl` | Latest 100,000 run-membership records; the append that would cross the bound rewrites the file, dropping **whole runs** oldest first | Automatic. Whole runs rather than individual records, because a half-present run would report a membership count lower than the truth, and a number wrong in the reassuring direction is exactly what this store exists to remove. A dropped run falls back to the durable-row scan — the pre-#380 answer — so the loss is a return to the old behaviour for a run 100,000 nodes old, not a wrong one. Never hand-edit it while the daemon is running; repair a damaged line with the daemon stopped. |
